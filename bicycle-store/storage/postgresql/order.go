...
// ------------------------------------------------------------------------------------------------------------
func (r *orderRepo) AddOrderItem(ctx context.Context, req *models.CreateOrderItem) (string, error) {
	...
	var biQ models.ReadStockBigQ

	q := `select store_id , product_id , quantity from stocks WHERE  product_id = $1 ORDER BY quantity DESC limit 1`

	er := r.db.QueryRow(ctx, q, req.ProductId).Scan(
		&biQ.Store_id,
		&biQ.Product_id,
		&biQ.Quantity,
	)
	...
	if biQ.Quantity == 0 {
		return "Товар не найден", er

	}

	fmt.Println(biQ)

	if req.Quantity < biQ.Quantity {

		biQ.Quantity -= req.Quantity
		...
		return "chopilish uchun quantity kopli qivotti ", er

	}

	return "uspeshno chopildi", nil
}
